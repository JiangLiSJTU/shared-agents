...
0. **DO NOT USE BROWSER**: Never use a browser subagent or web UI automation to interact with NotebookLM. Always use the API-based MCP tools or CLI listed below.
...
0. **🛑 NEVER USE BROWSER AUTOMATION**: DO NOT use the browser agent or try to open `notebooklm.google.com` in a web browser. The web UI actively blocks/fails for automated agents. You MUST exclusively use the structured `mcp_notebooklm_*` tools (or the `nlm` CLI) for ALL NotebookLM interactions.
